...
export async function GET(_request: NextRequest) {
  try {
    ...
    const result = await service.getLowStockItems(context.companyId);
    ...
    return errorResponse((error as Error).message || "Failed to fetch dashboard inventory alerts", "INTERNAL_ERROR", 500);
  }
}
